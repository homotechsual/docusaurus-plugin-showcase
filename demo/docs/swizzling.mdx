...
The plugin page renders card instances through the `@theme/ShowcaseCard` alias. This means any consumer override of `ShowcaseCard` is used consistently across showcase list rendering (favourites, filtered results, and all-items).

`ShowcasePage` does not attach page-level click handlers or navigation wrappers around cards. A swizzled `ShowcaseCard` can fully own interaction patterns such as:

- opening overlays or modals
- replacing link-based navigation
- custom analytics and click handling

When implementing a modal in a swizzled card, follow accessible dialog semantics:

- use `role="dialog"` and `aria-modal="true"`
- provide `aria-labelledby` (and `aria-describedby` when appropriate)
